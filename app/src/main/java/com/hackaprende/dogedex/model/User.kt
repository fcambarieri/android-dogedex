...
class User(
    ...
    companion object {
        ...
        fun getLoggedInUser(activity : Activity) : User? {
            val pref = activity.getSharedPreferences(AUTH_PREFS, Context.MODE_PRIVATE) ?: return null
            if (pref.getLong(ID_KEY, 0) == 0L) {
                return null
            }
            return User(pref.getLong(ID_KEY, 0), pref.getString(EMAIL_KEY, "") ?: "", pref.getString(
                AUTH_TOKEN_KEY, "")?: "")
        }

        fun logout(activity : Activity) {
            activity.getSharedPreferences(AUTH_PREFS, Context.MODE_PRIVATE)?.edit()?.clear()?.apply()
        }
    }
}